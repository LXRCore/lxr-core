--[[
    ...
    🐺 LXR Core - Interaction Prompt System
    
    Context-sensitive interaction prompts using native RedM prompt system
    with support for markers, distances, and grouped prompt sets.
...
local function createPrompt(name, coords, key, text, options, marker)
    if (Prompts[name] == nil) then
        ...
        print('[lxr-core]  Prompt with name ' .. name .. ' already exists!')
    end
end
...
local function createPromptGroup(group, label, coords, prompts)
    if (PromptGroups[group] == nil) then
        ...
        print('[lxr-core]  Prompt with name ' .. group .. ' already exists!')
    end
end
