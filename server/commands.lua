--[[
    ...
    🐺 LXR Core - Admin & Player Commands
    
    All server-side commands including admin tools, player management,
    economy controls, and utility commands with ACE permission integration.
...
CreateThread(function() -- Add ace to node for perm checking
    for k,v in pairs(LXRConfig.Permissions) do
        ExecuteCommand(('add_ace lxrcore.%s %s allow'):format(v, v))
    end
end)
...
local function AddCommand(name, help, arguments, argsrequired, callback, permission)
    ...
    if not IgnoreList[permission] then -- only create aces for extra perm levels
        ExecuteCommand(('add_ace lxrcore.%s command.%s allow'):format(permission, name))
    end
...
AddCommand('tp', 'TP To Player or Coords (Admin Only)', { { name = 'id/x', help = 'ID of player or X position' }, { name = 'y', help = 'Y position' }, { name = 'z', help = 'Z position' } }, false, function(source, args)
    ...
    if args[1] and not args[2] and not args[3] then
        ...
        if target ~= 0 then
            ...
            TriggerClientEvent('lxrcore:Command:TeleportToPlayer', src, coords)
        else
        ...
        if args[1] and args[2] and args[3] then
            ...
            if (x ~= 0) and (y ~= 0) and (z ~= 0) then
                TriggerClientEvent('lxrcore:Command:TeleportToCoords', src, x, y, z)
            else
                TriggerClientEvent('lxrcore:Notify', src, 9, Lang:t('error.wrong_format'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        else
            TriggerClientEvent('lxrcore:Notify', src, 9, Lang:t('error.missing_args'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
    end
end, 'admin')
...
AddCommand('togglepvp', 'Toggle PVP on the server (Admin Only)', {}, false, function(source)
    local pvp_state = LXRConfig.EnablePVP
    LXRConfig.EnablePVP = not pvp_state
    TriggerClientEvent('lxrcore:Client:PvpHasToggled', -1, LXRConfig.EnablePVP)
end, 'god')
...
AddCommand('car', 'Spawn Vehicle (Admin Only)', { { name = 'model', help = 'Model name of the vehicle' } }, true, function(source, args)
    local src = source
    TriggerClientEvent('lxrcore:Command:SpawnVehicle', src, args[1])
end, 'admin')
...
AddCommand('horse', 'Spawn Horse (Admin Only)', { { name = 'model', help = 'Model name of the horse' } }, true, function(source, args)
    local src = source
    TriggerClientEvent('lxrcore:Command:SpawnHorse', src, args[1])
end, 'admin')

AddCommand('coords', 'Get your current coords (Admin Only)', {}, false, function(source)
    local src = source
    TriggerClientEvent('lxrcore:Command:GetCoords', src)
end, 'user')
...
AddCommand("givexp", "Give A Player Xp (Admin Only)", {{name="id", help="Player ID"},{name="skill", help="Type of skill (mining, etc)"}, {name="amount", help="Amount of xp"}}, true, function(source, args)
	...
	if Player then
		if Player.PlayerData.metadata["xp"][tostring(args[2])] then
			...
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('info.xp_added'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
		else
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('error.no_skill'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	else
...
AddCommand("removexp", "Give A Player Xp (Admin Only)", {{name="id", help="Player ID"},{name="skill", help="Type of skill (mining, etc)"}, {name="amount", help="Amount of xp"}}, true, function(source, args)
	...
	if Player then
		if Player.PlayerData.metadata["xp"][tostring(args[2])] then
			...
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('info.xp_removed'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
		else
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('error.no_skill'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	else
...
AddCommand("xp", "Check How Much Xp You Have", {{name="skill", help="Type of skill (mining, etc)"}}, true, function(source, args)
	...
	if Player then
		if Xp then
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('info.xp_info', {value = Xp, value2 = tostring(args[1])}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
		else
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('error.no_skill'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	end
...
AddCommand("level", "Check Which Level You Are", {{name="skill", help="Type of skill (mining, etc)"}}, true, function(source, args)
	...
	if Player then
		if Level then
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('info.level_info', {value = Level, value2 = tostring(args[1])}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
		else
			TriggerClientEvent('lxrcore:Notify', source, 9, Lang:t('error.no_skill'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
		end
	end
...
AddCommand('job', 'Check Your Job', {}, false, function(source)
    ...
    TriggerClientEvent('lxrcore:Notify', src, 9, Lang:t('info.job_info', {value = PlayerJob.label, value2 = PlayerJob.grade.name, value3 = PlayerJob.onduty}), 5000, 0, 'toasts_mp_generic', 'butcher_table_production', 'COLOR_WHITE')
end, 'user')
...
AddCommand('gang', 'Check Your Gang', {}, false, function(source)
    ...
    TriggerClientEvent('lxrcore:Notify', src, 9, Lang:t('info.gang_info', {value = PlayerGang.label, value2 = PlayerGang.grade.name}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
end, 'user')
...
-- Inventory (should be in lxr-inventory?)
    ...
    for k, v in pairs(Players) do
        ...
        elseif HasPermission(v, 'admin') then
            if IsOptin(v) then
                ...
                TriggerEvent('lxr-log:server:CreateLog', 'ooc', 'OOC', 'white', '**' .. GetPlayerName(src) .. '** (CitizenID: ' .. Player.PlayerData.citizenid .. ' | ID: ' .. src .. ') **Message:** ' .. message, false)
            end
        end
    end
end, 'user')
