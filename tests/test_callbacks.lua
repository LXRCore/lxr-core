...
T.test('Register (return style) answers a client request by request id', function()
    ...
    Shim.netEvent(7, 'lxr:rpc:request', 'test:sum', 'req-1', 2, 3)
    local ev = Shim.clientEventsNamed('lxr:rpc:response')
    T.eq(#ev, 1)
    T.eq(ev[1].target, 7)
...
T.test('CreateCallback (RSG cb style) works and double-answers are ignored', function()
    ...
    Shim.netEvent(7, 'lxr:rpc:request', 'test:legacy', 'req-2', 21)
    local ev = Shim.clientEventsNamed('lxr:rpc:response')
    T.eq(#ev, 1)
    T.eq(ev[1].args[3], 42)
end)
...
T.test('unknown callback and rate limiting respond with failure', function()
    Shim.clientEvents = {}
    Shim.netEvent(8, 'lxr:rpc:request', 'test:missing', 'req-3')
    local ev = Shim.clientEventsNamed('lxr:rpc:response')
    ...
    for i = 1, Config.Security.callbackRateLimit.burst + 5 do
        Shim.netEvent(9, 'lxr:rpc:request', 'test:sum', 'r' .. i, 1, 1)
    end
    local limited = 0
    for _, e in ipairs(Shim.clientEventsNamed('lxr:rpc:response')) do
...
T.test('erroring callback still responds (nil) and is logged', function()
    ...
    Shim.netEvent(7, 'lxr:rpc:request', 'test:boom', 'req-4')
    local ev = Shim.clientEventsNamed('lxr:rpc:response')
    T.eq(#ev, 1)
    T.eq(ev[1].args[2], true)
...
T.test('client callback: Trigger resolves on response, times out otherwise, rejects spoofed source', function()
    ...
    local req = Shim.clientEventsNamed('lxr:rpc:ask')[1]
    T.eq(req.target, 11)
    T.eq(req.args[1], 'client:ask')
    local reqId = req.args[2]
    Shim.netEvent(12, 'lxr:rpc:answer', reqId, 'spoof')
    T.eq(got, nil, 'response from another source ignored')
    Shim.netEvent(11, 'lxr:rpc:answer', reqId, 'answer')
    T.eq(got, 'answer')
...
T.test('Await yields until answered', function()
    ...
    local req = Shim.clientEventsNamed('lxr:rpc:ask')[1]
    Shim.netEvent(11, 'lxr:rpc:answer', req.args[2], 'x', 'y')
    Shim.advance(5)
